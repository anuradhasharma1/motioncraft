...
export default function Playground() {
  ...
  const easingOptions = [
    'linear', 'easeIn', 'easeOut', 'easeInOut', 'backInOut', 'circInOut'
  ] as const;
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-20">
          <div className="space-y-12">
            <div>
              ...
              <p className="text-zinc-400 text-lg leading-relaxed">
                Connect the dots. Experiment with durations, delays, and transforms to see how they come together in real CSS.
              </p>
            </div>

            <div className="space-y-8">
              {[
                { label: 'Duration (s)', key: 'duration', min: 0.1, max: 5, step: 0.1 },
                { label: 'Delay (s)', key: 'delay', min: 0, max: 2, step: 0.1 },
                { label: 'Rotate (deg)', key: 'rotate', min: -360, max: 360, step: 1 },
                { label: 'Scale', key: 'scale', min: 0.2, max: 2, step: 0.1 },
                { label: 'Y-Offset (px)', key: 'y', min: -200, max: 200, step: 1 },
              ].map((control) => (
                <div key={control.key} className="space-y-4">
                  <div className="flex justify-between items-center text-[10px] uppercase font-bold tracking-widest text-zinc-500">
                    ...
                    <span className="text-white font-mono">{params[control.key as keyof typeof params]}</span>
                  </div>
                  <input
                    type="range"
                    min={control.min}
              ...
              <div className="space-y-4">
                <div className="text-[10px] uppercase font-bold tracking-widest text-zinc-500">Easing Function</div>
          ...
          <div className="flex flex-col gap-8">
            <div className="flex-1 bg-zinc-900 rounded-3xl border border-zinc-800 flex items-center justify-center p-20 relative overflow-hidden">
              ...
              <button 
                onClick={() => setParams({ duration: 1.5, delay: 0, rotate: 0, scale: 1, y: 0, ease: 'easeInOut' })}
            ...
            <div className="bg-zinc-900 rounded-3xl border border-zinc-800 p-8 space-y-6">
              <div className="flex justify-between items-center">
                <div className="text-[10px] uppercase font-bold tracking-widest text-zinc-500">CSS Output</div>
                <button 
